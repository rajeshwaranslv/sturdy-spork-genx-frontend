...
import { getTables, getOrders } from '../services/api';
...
function TableSelection({ history }) {
  ...
  // ✅ Count of pending orders for this waiter
  const pendingOrdersCount = orders.filter(
    (order) => order.waiterName === waiter.name && order.status === 'pending'
  ).length;
      ...
      <div className="waiter-card">
        ...
        <div className="waiter-right">
          ...
          <div className="orders-count">
            Orders: <span className="order-red">{pendingOrdersCount}</span>
          </div>
        </div>
      </div>
