...
import { Switch, Dropdown, Menu, Checkbox, InputNumber } from 'antd';
import './order.css';
...
function OrderReview({ match, history }) {
    ...
    try {
      ...
      setOrder((prev) => ({
        ...
        items: prev.items.map((item) =>
          item.id === itemId
            ? { ...item, status: checked ? 'served' : 'ordered' }
            : item
        ),
      }));
    } catch (err) {
  ...
  const updateQuantity = (itemId, newQty) => {
    ...
    // updateOrderItemQuantity(orderId, itemId, newQty);
  };
    ...
    <Menu>
      <Menu.Item>
        <Checkbox
          ...
          onChange={(e) =>
            setFilter({ ...filter, ordered: e.target.checked })
          }
        >
          Ordered
        </Checkbox>
      ...
      <Menu.Item>
        <Checkbox
          ...
          onChange={(e) =>
            setFilter({ ...filter, served: e.target.checked })
          }
        >
          Served
        </Checkbox>
      ...
      <div className="table-section">
        ...
        {filteredItems.length === 0 ? (
          <p style={{ textAlign: 'center' }}>
            No items to display for selected filter.
          </p>
        ) : (
        ...
        <button
          ...
          onClick={() => history.push(`/billing/${order.id}`)}
        >
          Generate Bill
        </button>
      </div>
