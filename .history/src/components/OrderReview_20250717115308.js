...
import { Switch, Dropdown, Menu, Checkbox } from 'antd';
import './order.css';
...
function OrderReview({ match, history }) {
    ...
    <Menu>
      <Menu.Item>
        <Checkbox
          ...
          onChange={(e) => setFilter({ ...filter, ordered: e.target.checked })}
        >
          Ordered
        </Checkbox>
      </Menu.Item>
      <Menu.Item>
        <Checkbox
          checked={filter.served}
          onChange={(e) => setFilter({ ...filter, served: e.target.checked })}
        >
          Served
        </Checkbox>
      </Menu.Item>
  ...
  const filteredItems = order.items.filter(item =>
    (item.status === 'ordered' && filter.ordered) ||
    (item.status === 'served' && filter.served)
  );
    ...
    <div className="container-fluid">
      ...
      <div className="header-bar">
        ...
        <Dropdown overlay={filterMenu} trigger={['click']}>
          <FaFilter className="icon" style={{ cursor: 'pointer' }} />
        </Dropdown>
      </div>
      ...
      <div className="waiter-card">
        ...
        <div className="waiter-right">
          ...
          <div className="orders-count">
            <span className="order-red">{filteredItems.length.toString().padStart(2, '0')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="table-section">
        ...
        {filteredItems.length === 0 ? (
          <p style={{ textAlign: 'center' }}>No items to display for selected filter.</p>
        ) : (
          filteredItems.map((item) => (
            <div key={item.id} className="item-row">
              <div className="item-info" style={{ marginLeft: '1rem', marginBottom: '1rem' }}>
