...
function OrderReview({ match, history }) {
  ...
  // ✅ FIX: Toggle item status and update UI locally
  const toggleStatus = async (itemId, checked) => {
    try {
      ...
      // Update the UI locally without refetching the entire order
      setOrder((prev) => ({
        ...prev,
  ...
  if (!order || !order.items || order.items.length === 0) {
    return <div className="container"><p>No items in this order.</p></div>;
  }
        ...
        {order.items.map((item) => (
          <div key={item.id} className="item-row">
            <div className="item-info">
              ...
              <p>₹ {(item.price || 0).toFixed(2)} x {item.quantity}</p>
            </div>
